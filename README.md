...
Call the `dump` API as a message from Agent by specifying the message, `clientId` (represented as '3' below) `message type` and whether the dump is from an audio call or not (`isCall`) as per below;
