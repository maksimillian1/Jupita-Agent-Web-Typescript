...
The required parameters for the APIs include setting 'message type', along with assigning a 'agent' + 'client' ID to be passed through to the APIs. How this is structured or deployed is completely dependant on your systems/platforms architcure, therefore it is completely flexible and customisable. Please note when assigning the 'agent' ID, no data will be available for that particular agent from any APIs until the agent has sent at least 1 utterance via the 'dump' API. 
